...
function train_scaled(){
    ...
    dataset=wt103
    datadir=$(python -c "print('../lm-data/wikitext-103-'+str($dsdiv)+'/')")
    ...
    fairseq-train --fp16 $DATA_DIR \
        ...
        --encoder-ffn_embed-dim $encoder_ffn_embed_dim \
        --encoder-attention-heads $encoder_attention_heads \
        --sample-break-mode complete \
        --tokens-per-sample $TOKENS_PER_SAMPLE \
        --optimizer adam --adam-betas '(0.9,0.98)' ֿ--adam-eps 1e-6 --clip-norm 0.0 \
